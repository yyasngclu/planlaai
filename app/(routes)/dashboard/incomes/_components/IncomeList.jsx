    ...
    <div className="mt-7">
      ...
      {editModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <form onSubmit={handleEditSubmit} className="bg-white p-6 rounded shadow w-full max-w-md">
            <h2 className="text-lg font-bold mb-4">Geliri Düzenle</h2>
            <div className="mb-2">
              <label className="block mb-1">Ad</label>
              <input type="text" value={editIncomeData.name} onChange={e => setEditIncomeData({ ...editIncomeData, name: e.target.value })} className="w-full border px-2 py-1 rounded" />
            </div>
            <div className="mb-2">
              <label className="block mb-1">Tutar</label>
              <input type="number" value={editIncomeData.amount} onChange={e => setEditIncomeData({ ...editIncomeData, amount: e.target.value })} className="w-full border px-2 py-1 rounded" />
            </div>
            <div className="mb-2">
              <label className="block mb-1">İkon</label>
              <input type="text" value={editIncomeData.icon} onChange={e => setEditIncomeData({ ...editIncomeData, icon: e.target.value })} className="w-full border px-2 py-1 rounded" />
            </div>
            <div className="flex gap-2 mt-4">
              <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded">Kaydet</button>
              <button type="button" className="px-4 py-2 bg-gray-300 rounded" onClick={() => setEditModalOpen(false)}>İptal</button>
            </div>
          </form>
        </div>
      )}

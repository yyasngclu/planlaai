        ...
        <div className="flex gap-2 items-center justify-between">
          <div className="flex gap-2 items-center">
            ...
            <div>
              ...
              {budget.goal && (
                <h2 className="text-xs text-blue-500">Hedef: {budget.goal}</h2>
              )}
        ...
        <div className="mt-3 flex flex-col gap-1">
          {budget.incomes && budget.incomes.length > 0 && (
            <h2 className="text-xs text-green-600">
              Toplam Gelir: ₺
              {budget.incomes.reduce((acc, i) => acc + Number(i.amount), 0)}
            </h2>
          )}
          {budget.expenses && budget.expenses.length > 0 && (
            <h2 className="text-xs text-red-600">
              Toplam Gider: ₺
              {budget.expenses.reduce((acc, e) => acc + Number(e.amount), 0)}
            </h2>
          )}
        </div>
      </div>
